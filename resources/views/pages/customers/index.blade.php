...
@section('content')
    ...
    @can('v_user_workers')
        ...
        {{-- Users Table --}}
        <h1 class="text-center">Worker's Management</h1>
        <p class="text-center text-primary">
            Here You can see all your Workers.
        </p>
        <div class="row">
            <div class="row py-3 text-center text-small text-muted">
                <div class="col"><p class="m-0">avatar</p></div>
                <div class="col"><p class="m-0">User Name</p></div>
                <div class="col"><p class="m-0">User Email</p></div>
            ...
            @foreach($workers as $worker)
                <div class="row text-muted text-center bg-white c-it border border-primary anime my-2 shadow-md hover-up rounded-lg">
                    ...
                    <div class="col py-2">
                        <x-avatar class="shadow-sm" :for="$worker->profile->avatar" radius="10%" w="4rem" h="4rem"/>
                    </div>
                    <div class="col py-1">{{ $worker->profile->user_name }}</div>
                    <div class="col py-1">{{ $worker->email }}</div>
                    <div class="col py-1">
                        <h4 class="m-0">
                            <span class="badge alert m-0 p-1 alert-{{ $worker->profile->status == 'online' ? 'success' : 'warning' }}">
                                {{ $worker->profile->status }}
                            </span>
                        </h4>
                    </div>
                    <div class="col py-1">
                        <a href="/customers/{{$worker->id}}/edit" class="btn btn-link text-decoration-none ml-2 btn-primary rounded-lg shadow-sm-primary hovered">
                            <x-icon i="userCog" class="m-0 p-0" h="1.5rem" w="1.5rem"/>
                        </a>
                    </div>
